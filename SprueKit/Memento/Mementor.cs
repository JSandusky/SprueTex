namespace Memento
{
    ...
    public class Mementor : IDisposable
    {
        ...
        public System.Collections.Generic.List<BaseEvent> InlineStack
        {
            get
            {
                System.Collections.Generic.List<BaseEvent> ret = new System.Collections.Generic.List<BaseEvent>();
                ret.AddRange(_undoStack);
                ret.AddRange(_redoStack);
                return ret;
            }
        }

        /// <summary>
        /// Checks to see if the given event is the most recent action
        /// </summary>
        /// <param name="@event">Event whose status should be checked</param>
        /// <returns></returns>
        public bool IsCurrent(BaseEvent @event)
        {
            if (_undoStack.Count > 0)
                return _undoStack.Peek() == @event;
            return false;
        }
        ...
        /// <summary>
        /// If <c>true</c>, all calls to mark changes are ignored. Checks for locker status is disabling
        /// </summary>
        /// <seealso cref="ExecuteNoTrack"/>
        bool trackingEnabled_ = true;
        public bool IsTrackingEnabled { get { return trackingEnabled_ && !MementorLocker.locked; } set { trackingEnabled_ = value; } }
