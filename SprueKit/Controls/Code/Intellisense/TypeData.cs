...
namespace SprueKit.Controls.Code.Intellisense
{
    ...
    public class PropInfo : PossiblyIncomplete, TypeImageSource
    {
        ...
        public String PrettyName
        {
            ...
            {
                if (WrappedType != null)
                    return String.Format("{0}<{1}>", Type.Name, WrappedType.Name);
                if (Type is TemplateInst)
                    return String.Format("{0}<{1}>", Type.Name, ((TemplateInst)Type).WrappedType.Name);
                return Type.Name;
            }
        }
    ...
    public class FunctionInfo : BaseTypeInfo, PossiblyIncomplete, TypeImageSource
    {
        ...
        public string PrettyName
        {
            get
            {
                ...
                {
                    if (((TemplateInst)ReturnType).WrappedType != null)
                        return String.Format("{0}<{1}>", ReturnType.Name, ((TemplateInst)ReturnType).WrappedType.Name);
    ...
    public class Globals : BaseTypeInfo
    {
        ...
        public void MergeIntoThis(Globals other)
        {
            foreach (string k in other.Classes.Keys)
            {
                ...
                //TODO: if (!Classes.ContainsKey(k))
                //TODO:     Classes[k] = IDEProject.inst().GlobalTypes.Classes[k];
            }
        }
