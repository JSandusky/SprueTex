...
namespace SprueKit.Controls.Code.Intellisense
{
    ...
    public static class IntellisenseHelper
    {
        ...
        /// <summary>
        /// Makes a "rough guess" check if the string of text found at the position resembles a dot path, object.member.childMember
        /// </summary>
        ...
        /// <summary>
        /// Extracts a "path" from the AvalonEdit document
        /// </summary>
        ...
        //Get's the location to place the caret for jumping over a word to the left
        ...
        public static bool ScanFor(TextEditor editor, char code, char[] exclusions)
        {
            int offset = editor.CaretOffset - 1;
            while (offset > 0)
            {
                char k = editor.Document.GetCharAt(offset);
                if (k == code)
                    return true;
                else if (exclusions.Contains(k))
                    return false;
            }
            return false;
        }
