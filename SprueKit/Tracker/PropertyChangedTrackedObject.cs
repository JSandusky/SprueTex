namespace Notify
{
    ...
    /// </list>
    /// </summary>
    public class PropertyChangedTrackedObject : TrackedObject
    {
        private readonly Dictionary<object, TrackedObject> _registeredProperties = new Dictionary<object, TrackedObject>();
        public object Snapshot;
        public TrackedObject Owner;

        internal PropertyChangedTrackedObject(object tracked, TrackedObject owner)
            : base(tracked)
        {
            Owner = owner;
            Snapshot = tracked.BasicClone();
        }
        ...
        private void OnPropertyChanged(object sender, PropertyChangedEventArgs args)
        {
            ...
            if (property != null && IsEligibleProperty(property, attribute))
            {
                ...
                OnChange(null, this, args.PropertyName);

                // Update our snapshot object
                Snapshot = sender.BasicClone();
            }
        }
        ...
        private bool IsEligibleProperty(PropertyInfo property, TrackClassAttribute trackClassAttr)
        {
            ...
            if (!property.CanWrite && !(typeof(System.Collections.Specialized.INotifyCollectionChanged).IsAssignableFrom(property.PropertyType)))
                return false;
        ...
        private void RegisterProperty(object obj, PropertyInfo property)
        {
            ...
            if (IsValidObjectType(propValue))
            {
                ...
                var trackedObject = Create(propValue, this);
