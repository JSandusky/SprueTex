...
bool BasicImageLoader::ReadPNGMemory(FilterableBlockMap<RGBA>* target, const char* buffer, int dataLength)
{
    int width = 0;
    int height = 0;
    int comps = 0;
    if (unsigned char* data = stbi_load_from_memory((const unsigned char*)buffer, dataLength, &width, &height, &comps, 3))
    {
        target->resize(width, height);
        for (unsigned y = 0, idx = 0; y < height; ++y)
        {
            for (unsigned x = 0; x < width; ++x, idx += comps)
                target->set(RGBA::FromData(data, idx, comps), x, y);
        }
        stbi_image_free(data);
        return true;
    }
...
{
    const bool anyAlpha = true;// AnyAlphaUsed(image);
