...
namespace SprueEngine
{
    ...
    enum RemappingMode
    {
        ...
        REMAP_SagitalRelative = 1 << 5, // 0.0 = sagital plane, 1.0 equals +X bounding box face, -1.0f equals -X bounding box face
                                            // Gx = Normalize(Px, 0, halfBoundsX)
                                            // Sx = Denormalize(Px, 0, halfBoundsX)
        REMAP_CoronalRelative = 1 << 5, // 0.0 = coronal plane, 1.0 equals +Z bounding box face, -1.0f equals -Z bounding box face
                                            // Gx = Normalize(Pz, 0, halfBoundsZ)
                                            // Sx = Denormalize(Pz, 0, halfBoundsZ)
        REMAP_ForeignRelative = 1 << 7, // Relative to another position
                                            // G = Normalize(P, R, Of)      // Of - R
                                            // S = Denormalize(P, R, Nf)    // R + Nf
        REMAP_LookAt = 1 << 8,          // Look at the target
                                            // G = Normalized(P - R)
