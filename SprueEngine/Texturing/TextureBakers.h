...
// return false if needs to cancel, will be given the current % (if possible)
typedef bool(*BAKER_CANCELATION)(float);
...
public:
    ...
    virtual FilterableBlockMap<RGBA>* Bake(FilterableBlockMap<RGBA>* into = 0x0, BAKER_CANCELATION cancelFunc = 0x0) const = 0;
...
#define DECL_TEXTURE_BAKER(TYPENAME) class SPRUE TYPENAME : public TextureBaker { BASECLASSDEF(TYPENAME, TextureBaker); NOCOPYDEF(TYPENAME); public: TYPENAME(nv::HalfEdge::Mesh* mesh, MeshData* meshData) : base(mesh, meshData) { } virtual FilterableBlockMap<RGBA>* Bake(FilterableBlockMap<RGBA>* into = 0x0, BAKER_CANCELATION cancelFunc = 0x0) const override; }
...
class ThicknessBaker : public TextureBaker
{
    BASECLASSDEF(ThicknessBaker, TextureBaker);
public:
    ThicknessBaker(nv::HalfEdge::Mesh* mesh, MeshData* meshData) :
        base(mesh, meshData),
        scalingFactor_(1.0f)
    {
    }
