...
FilterableBlockMap<RGBA>* AmbientOcclusionBaker::Bake(FilterableBlockMap<RGBA>* into, BAKER_CANCELATION cancelFunc) const
{
    FilterableBlockMap<RGBA>* ret = into ? into : new FilterableBlockMap<RGBA>(GetWidth(), GetHeight());
    if (into == 0x0)
        into->fill(RGBA(0, 0, 0, 0));
    DECL_RASTER;
    ...
    const float ct = meshData_->indexBuffer_.size();
    for (unsigned i = 0; i < meshData_->indexBuffer_.size(); i += 3)
    {
        if (cancelFunc && !cancelFunc(i * 3 / ct))
            return ret;
...
FilterableBlockMap<RGBA>* ThicknessBaker::Bake(FilterableBlockMap<RGBA>* into, BAKER_CANCELATION cancelFunc) const
{
    FilterableBlockMap<RGBA>* ret = into ? into : new FilterableBlockMap<RGBA>(GetWidth(), GetHeight());
    if (into == 0x0)
        into->fill(RGBA(0, 0, 0, 0));
...
FilterableBlockMap<RGBA>* CurvatureBaker::Bake(FilterableBlockMap<RGBA>* into, BAKER_CANCELATION cancelFunc) const
{
    FilterableBlockMap<RGBA>* ret = into ? into : new FilterableBlockMap<RGBA>(GetWidth(), GetHeight());
    DECL_RASTER;
...
FilterableBlockMap<RGBA>* DominantPlaneBaker::Bake(FilterableBlockMap<RGBA>* into, BAKER_CANCELATION cancelFunc) const
{
    FilterableBlockMap<RGBA>* ret = into ? into : new FilterableBlockMap<RGBA>(GetWidth(), GetHeight());
    DECL_RASTER;
...
FilterableBlockMap<RGBA>* ObjectSpaceNormalBaker::Bake(FilterableBlockMap<RGBA>* into, BAKER_CANCELATION cancelFunc) const
{
    FilterableBlockMap<RGBA>* ret = into ? into : new FilterableBlockMap<RGBA>(GetWidth(), GetHeight());
    DECL_RASTER;
...
FilterableBlockMap<RGBA>* ObjectSpacePositionBaker::Bake(FilterableBlockMap<RGBA>* into, BAKER_CANCELATION cancelFunc) const
{
    FilterableBlockMap<RGBA>* ret = into ? into : new FilterableBlockMap<RGBA>(GetWidth(), GetHeight());
    ret->fill(RGBA::Clear);
    DECL_RASTER;
...
FilterableBlockMap<RGBA>* ObjectSpaceGradientBaker::Bake(FilterableBlockMap<RGBA>* into, BAKER_CANCELATION cancelFunc) const
{
    FilterableBlockMap<RGBA>* ret = into ? into : new FilterableBlockMap<RGBA>(GetWidth(), GetHeight());
    ret->fill(RGBA::Clear);
    DECL_RASTER;
...
FilterableBlockMap<RGBA>* VertexColorBaker::Bake(FilterableBlockMap<RGBA>* into, BAKER_CANCELATION cancelFunc) const
{
    FilterableBlockMap<RGBA>* ret = into ? into : new FilterableBlockMap<RGBA>(GetWidth(), GetHeight());
    DECL_RASTER;
...
FilterableBlockMap<RGBA>* FacetBaker::Bake(FilterableBlockMap<RGBA>* into, BAKER_CANCELATION cancelFunc) const
{
    FilterableBlockMap<RGBA>* ret = into ? into : new FilterableBlockMap<RGBA>(GetWidth(), GetHeight());
    DECL_RASTER;
    ...
    const float ct = meshData_->indexBuffer_.size() * 2;
    for (unsigned i = 0; i < meshData_->indexBuffer_.size(); i += 3)
    {
        if (cancelFunc && !cancelFunc(i * 3 / ct))
            return ret;
...
FilterableBlockMap<RGBA>* PlanarProjectionBaker::Bake(FilterableBlockMap<RGBA>* into, BAKER_CANCELATION cancelFunc) const
{
    FilterableBlockMap<RGBA>* ret = into ? into : new FilterableBlockMap<RGBA>(GetWidth(), GetHeight());
    DECL_RASTER;
...
FilterableBlockMap<RGBA>* TriPlanarProjectionBaker::Bake(FilterableBlockMap<RGBA>* into, BAKER_CANCELATION cancelFunc) const
{
    FilterableBlockMap<RGBA>* ret = into ? into : new FilterableBlockMap<RGBA>(GetWidth(), GetHeight());
    DECL_RASTER;
