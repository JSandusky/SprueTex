...
#define nvDebugCheck(CHECK) assert(CHECK)
...
Mesh::Mesh(const Mesh * mesh)
{
    ...
    for (uint32_t v = 0; v < vertexCount; v++)
    {
        ...
        SPRUE_ASSERT(vertex->id == v, "Inappropriate vertex id in HalfEdge::Mesh::Mesh(Mesh*)");
...
void Mesh::clear()
{
    for (auto i : m_vertexArray)
        delete i;
    m_vertexArray.clear();

    for (auto i : m_edgeMap)
        delete i.second;
    //deleteAll(m_edgeArray);	// edgeArray only contains 1/2 of the edges!
    m_edgeArray.clear();
    m_edgeMap.clear();

    for (auto i : m_faceArray)
        delete i;
    m_faceArray.clear();
}


Vertex * Mesh::addVertex(const SprueEngine::Vec3 & pos)
{
    //SPRUE_ASSERT(pos.IsFinite(), "HalfEdge::Mesh::addVertex() argument 'pos' must be finite");
...
/*Vertex * Mesh::addVertex(uint id, const SprueEngine::Vec3 & pos)
{
...
void Mesh::addVertices(const SprueEngine::Vec3* verts, uint32_t ct, const SprueEngine::Vec2* uv, const SprueEngine::Vec3* norms, const SprueEngine::RGBA* colors)
{
    for (uint32_t v = 0; v < ct; ++v)
    {
        if (!vertexAt(v))
            continue;
        Vertex* vert = addVertex(verts[v]);
        if (uv)
            vert->tex = uv[v];
        if (norms)
            vert->nor = norms[v];
...
void Mesh::linkColocals()
{
    ...
    std::map<SprueEngine::Vec3, Vertex *> vertexMap;

    for (uint32_t v = 0; v < vertexCount; v++)
    {
        Vertex * vertex = vertexAt(v);

        auto colocal = vertexMap.find(vertex->pos);
        if (colocal != vertexMap.end())
        {
            colocal->second->linkColocal(vertex);
        }
        else
        {
...
void Mesh::linkColocalsWithCanonicalMap(const std::vector<uint32_t> & canonicalMap)
{
    ...
    for (auto i : canonicalMap) {
        vertexMapSize = std::max(vertexMapSize, i + 1);
    }
...
Edge * Mesh::addEdge(uint32_t i, uint32_t j)
{
    ...
    if (edge != NULL) {
        ...
        m_edgeMap[std::pair<uint32_t, uint32_t>(i,j)] = edge;
    }
...
Edge * Mesh::findEdge(uint32_t i, uint32_t j) const
{
    ...
    for(Vertex::ConstVertexIterator it0(v0->colocals()); !it0.isDone(); it0.advance())
    {
        ...
        {
            std::pair<uint32_t,uint32_t> key(it0.current()->id, it1.current()->id);

            if (edge == NULL) {
                auto foundEdge = m_edgeMap.find(key);
                if (foundEdge != m_edgeMap.end())
                    edge = foundEdge->second;
#if !defined(_DEBUG)
...
void Mesh::linkBoundary()
{
    ...
    for (uint32_t e = 0; e < edgeCount; e++)
    {
        ...
        if (edge != NULL && edge->pair == NULL) {
            ...
            std::pair<uint32_t,uint32_t> key(j,i);
...
TriMesh * Mesh::toTriMesh() const
{
    ...
    const uint32_t faceCount = this->faceCount();
    for (uint32_t f = 0; f < faceCount; f++)
    {
    ...
    // Add vertices.
    std::vector<TriMesh::Vertex> & vertices = triMesh->vertices();
    ...
    // Add triangles.
    std::vector<TriMesh::Face> & triangles = triMesh->faces();
    ...
    for (uint32_t f = 0; f < faceCount; f++)
    {
        ...
        uint32_t v1 = face->edge->next->vertex->id;

        for(Face::ConstEdgeIterator it(face->edges()); !it.isDone(); it.advance())
        {
            uint32_t v2 = it.current()->vertex->id;
...
void Mesh::triangulate() {
    ...
    for (auto i : edgeArray) delete i;
    for (auto i : faceArray) delete i;
}
...
bool Mesh::splitBoundaryEdges() {
    ...
    for (unsigned vIdx = 0; vIdx < boundaryVertices.size(); ++vIdx) {
        Vertex * vertex = boundaryVertices[vIdx];

        SprueEngine::Vec3 x0 = vertex->pos;

        // Find edges that this vertex overlaps with.
        for (unsigned eIdx = 0; eIdx < m_edgeArray.size(); ++eIdx) {
            Edge * edge = m_edgeArray[eIdx];
            if (edge != NULL && edge->isBoundary()) {
                ...
                SprueEngine::Vec3 x1 = edge->from()->pos;
                SprueEngine::Vec3 x2 = edge->to()->pos;

                SprueEngine::Vec3 v01 = x0 - x1;
                SprueEngine::Vec3 v21 = x2 - x1;

                float l = v21.Length();
                float d = v01.Cross(v21).Length() / l;

                if (SprueEquals(d, 0)) {
                    float t = v01.Dot(v21) / (l * l);

                    // @@ Snap x0 to x1 or x2, if too close? No, do vertex snapping elsewhere.
                    if (SprueEqualsApprox(t, 0.0f, 0.01f)) {
                        vertex->setPos(x1);
                    }
                    else if (SprueEqualsApprox(t, 1.0f, 0.01f)) {
                        vertex->setPos(x2);
                    }
                    else if (t > 0.0f + FLT_EPSILON && t < 1.0f - FLT_EPSILON) {
                        //assert(SprueEquals(SprueLerp(x1, x2, t), x0));
...
void Mesh::disconnect(Edge * edge) {
    ...
    bool removed = m_edgeMap.erase(std::pair<uint32_t, uint32_t>(edge->from()->id, edge->to()->id));
...
Vertex * Mesh::splitBoundaryEdge(Edge * edge, float t, const SprueEngine::Vec3 & pos) {
    ...
    */
    ...
    vertex->nor = SprueLerp(edge->from()->nor, edge->to()->nor, t);
    vertex->tex = SprueLerp(edge->from()->tex, edge->to()->tex, t);
    vertex->col = SprueLerp(edge->from()->col, edge->to()->col, t);
...
void Mesh::Extrude(std::vector<Face*>& faces, const SprueEngine::Vec3& magnitutde)
{
    std::vector<Edge*> boundaryEdges;

    for (Face* face : faces)
    {
        auto edgeIt = face->edges();
        while (!edgeIt.isDone())
        {
            // check that it has an opposite edge and that the face for that edge isn't in our extrude list
            if (edgeIt.current()->pair && std::find(faces.begin(), faces.end(), edgeIt.current()->pair->face) == faces.end())
                boundaryEdges.push_back(edgeIt.current());
            edgeIt.advance();
        }
    }
...
void Mesh::Split(std::vector<Edge*>& edges)
{
