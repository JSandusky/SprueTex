...
namespace
{
    ...
    struct PriorityQueue
    {
        ...
        void sort() {
            ...
            std::sort(pairs.data(), pairs.data() + pairs.size());
        }
        ...
        uint32_t count() const { return pairs.size(); }
...
struct nv::ChartBuildData
{
    ChartBuildData(int id) : id(id) {
        planeNormal = SprueEngine::Vec3(0,0,0);
        centroid = SprueEngine::Vec3(0, 0, 0);
        coneAxis = SprueEngine::Vec3(0, 0, 0);
        coneAngle = 0;
        area = 0;
        boundaryLength = 0;
        normalSum = SprueEngine::Vec3(0, 0, 0);
        centroidSum = SprueEngine::Vec3(0, 0, 0);
    }

    int id;

    // Proxy info:
    SprueEngine::Vec3 planeNormal;
    SprueEngine::Vec3 centroid;
    SprueEngine::Vec3 coneAxis;
    float coneAngle;
    
    float area;
    float boundaryLength;
    SprueEngine::Vec3 normalSum;
    SprueEngine::Vec3 centroidSum;
...
void AtlasBuilder::computeShortestPaths()
{
    ...
    for (uint32_t i = 0; i < faceCount; i++)
    {
        ...
        SprueEngine::Vec3 centroid_i = face_i->centroid();
        ...
        for (HalfEdge::Face::ConstEdgeIterator it(face_i->edges()); !it.isDone(); it.advance())
        {
            ...
            if (!edge->isBoundary())
            {
                ...
                SprueEngine::Vec3 centroid_j = face_j->centroid();

                shortestPaths[i*faceCount + j] = shortestPaths[j*faceCount + i] = (centroid_i - centroid_j).Length();
            }
        }
    }
...
void AtlasBuilder::resetCharts()
{
    ...
    for (uint32_t i = 0; i < chartCount; i++)
    {
        ...
        chart->normalSum = SprueEngine::Vec3(0,0,0);
        chart->centroidSum = SprueEngine::Vec3(0,0,0);
...
namespace {
    ...
    float absoluteSum(SprueEngine::Vec4 v)
    {
    ...
    struct ConeFitting
    {
        ...
        void addTerm(SprueEngine::Vec3 N, float A)
        {
            const float c = cosf(X.w);
            const float s = sinf(X.w);
            const float tmp = X.xyz().Dot(N) - c;
        ...
        SprueEngine::Vec4 solve(ChartBuildData * chart, SprueEngine::Vec4 start)
        {
            ...
            do {
                ...
                X = SprueEngine::Vec4(X.xyz().Normalized(), X.w);
    ...
    static SprueEngine::Vec3 triangleNormal(const HalfEdge::Face * face)
    {
        SprueEngine::Vec3 p0 = face->edge->vertex->pos;
        SprueEngine::Vec3 p1 = face->edge->next->vertex->pos;
        SprueEngine::Vec3 p2 = face->edge->next->next->vertex->pos;

        SprueEngine::Vec3 e0 = p2 - p0;
        SprueEngine::Vec3 e1 = p1 - p0;

        return e0.Cross(e1).Normalized();
    }

    static SprueEngine::Vec3 triangleNormalAreaScaled(const HalfEdge::Face * face)
    {
        SprueEngine::Vec3 p0 = face->edge->vertex->pos;
        SprueEngine::Vec3 p1 = face->edge->next->vertex->pos;
        SprueEngine::Vec3 p2 = face->edge->next->next->vertex->pos;
    ...
    static SprueEngine::Vec3 triangleCenter(const HalfEdge::Face * face)
    {
        SprueEngine::Vec3 p0 = face->edge->vertex->pos;
        SprueEngine::Vec3 p1 = face->edge->next->vertex->pos;
        SprueEngine::Vec3 p2 = face->edge->next->next->vertex->pos;
        ...
        SprueEngine::Vec3 m0 = (p0 + p1) * l0 / (l0 + l1 + l2);
        SprueEngine::Vec3 m1 = (p1 + p2) * l1 / (l0 + l1 + l2);
        SprueEngine::Vec3 m2 = (p2 + p0) * l2 / (l0 + l1 + l2);
...
void AtlasBuilder::updateProxy(ChartBuildData * chart)
{
    ...
    chart->planeNormal = chart->normalSum.Normalized();
    ...
    SprueEngine::Vec4 start = SprueEngine::Vec4(chart->coneAxis, chart->coneAngle);
    SprueEngine::Vec4 solution = coneFitting.solve(chart, start);
...
{
    SprueEngine::Vec3 centroid = computeChartCentroid(chart);
    ...
    for (uint32_t i = 0; i < bestCount; i++)
    {
        ...
        SprueEngine::Vec3 faceCentroid = triangleCenter(face);

        float distance = (centroid - faceCentroid).Length();
    ...
    auto found = std::find(chart->seeds.begin(), chart->seeds.end(), mostCentral);
    if (found != chart->seeds.end())
    {
        uint32_t index = std::distance(chart->seeds.begin(), found);
...
void AtlasBuilder::updatePriorities(ChartBuildData * chart)
{
    ...
    uint32_t candidateCount = chart->candidates.size();
    for (uint32_t i = 0; i < candidateCount; i++)
    {
...
{
    const HalfEdge::Face * face = mesh->faceAt(f);
    SprueEngine::Vec3 faceNormal = triangleNormal(face);
    ...
    return 1 - faceNormal.Dot(chart->planeNormal); // @@ normal deviations should be weighted by face area

    // Find distance to chart.
    /*SprueEngine::Vec3 faceCentroid = face->centroid();
...
float AtlasBuilder::evaluateDistanceToSeed(ChartBuildData * chart, uint32_t f)
{
    ...
    const HalfEdge::Face * face = mesh->faceAt(f);
    return (triangleCenter(seed) - triangleCenter(face)).Length();
}
...
float AtlasBuilder::evaluateRoundnessMetric(ChartBuildData * chart, uint32_t face, float newBoundaryLength, float newChartArea)
{
    ...
    float roundness = SprueSquare(chart->boundaryLength) / chart->area;
    float newRoundness = SprueSquare(newBoundaryLength) / newChartArea;
    if (newRoundness > roundness) {
        return SprueSquare(newBoundaryLength) / (newChartArea * 4 * PI);
    }
    else {
...
float AtlasBuilder::evaluateNormalSeamMetric(ChartBuildData * chart, uint32_t f)
{
    ...
    for (HalfEdge::Face::ConstEdgeIterator it(face->edges()); !it.isDone(); it.advance())
    {
        ...
        {
            float d0 = CLAMP(edge->vertex->nor.Dot(edge->pair->next->vertex->nor), 0.0f, 1.0f);
            float d1 = CLAMP(edge->next->vertex->nor.Dot(edge->pair->vertex->nor), 0.0f, 1.0f);
...
SprueEngine::Vec3 AtlasBuilder::evaluateChartNormalSum(ChartBuildData * chart, uint32_t f)
{
    const HalfEdge::Face * face = mesh->faceAt(f);
    return chart->normalSum + triangleNormalAreaScaled(face);
}

SprueEngine::Vec3 AtlasBuilder::evaluateChartCentroidSum(ChartBuildData * chart, uint32_t f)
{
    const HalfEdge::Face * face = mesh->faceAt(f);
...
SprueEngine::Vec3 AtlasBuilder::computeChartCentroid(const ChartBuildData * chart)
{
    SprueEngine::Vec3 centroid(0,0,0);
...
void AtlasBuilder::mergeCharts()
{
    ...
    for (int c = chartCount-1; c >= 0; c--)
    {
        ...
        for (int cc = chartCount-1; cc >= 0; cc--)
        {
            ...
            if (sharedBoundaryLengths[cc] > 0.8 * std::max(0.0f, chart->boundaryLength - externalBoundary)) {
                ...
                {
                    if (chart2->planeNormal.Dot(chart->planeNormal) > -0.25) {
            ...
            if (sharedBoundaryLengths[cc] > 0.20 * std::max(0.0f, chart->boundaryLength - externalBoundary)) {
                ...
                // Compare proxies.
                if (chart2->planeNormal.Dot(chart->planeNormal) > 0) {
                    mergeChart(chart2, chart, sharedBoundaryLengths[cc]);
                    delete chart;
                    chartArray[c] = NULL;
                    break;
    ...
    for (int c = 0; c < chartArray.size(); /*do not increment if removed*/)
    {
        if (chartArray[c] == NULL) {
            ...
            for (uint32_t i = 0; i < faceCount; i++) {
                ...
                //nvDebugCheck (faceChartArray[i] <= I32(chartArray.size()));
