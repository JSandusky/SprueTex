...
namespace SprueEngine
{
    ...
    void MeshData::CalculateNormals()
    {
        ...
        //    normalAccumulation[record.first] = -record.second.Normalized();

        for (unsigned i = 0; i < indexBuffer_.size(); ++i)
        {
            const int i0 = indexBuffer_[i];
            Vec3& v0 = positionBuffer_[i0];
            normalBuffer_[i0] = normalAccumulation[v0].Normalized();
        }
    }
    ...
    void MeshData::SweepRayForWinding(const Ray& ray, unsigned& winding, const Mat3x4& trans) const
    {
        float nearest = std::numeric_limits<float>::max();
        unsigned nearestIndex = -1;
        ...
        for (unsigned i = 0; i < indexBuffer_.size(); i += 3)
        {
            Vec3 v0 = trans.MulPos(positionBuffer_[indexBuffer_[i]]);
            Vec3 v1 = trans.MulPos(positionBuffer_[indexBuffer_[i + 1]]);
            Vec3 v2 = trans.MulPos(positionBuffer_[indexBuffer_[i + 2]]);
    ...
    MeshData* MeshData::CloneRaw() const
    {
        ...
        ret->bounds_ = bounds_;
        ret->morphTargets_ = morphTargets_;
    ...
    void MeshData::Smooth(std::set<unsigned> indices, float strength)
    {
        ...
        std::vector<Vec3> normalCache = normalBuffer_;
        if (nv::HalfEdge::Mesh* mesh = BuildHalfEdgeMesh())
        {
            ...
            while (!vertIt.isDone())
            {
                ...
                while (!edgeIt.isDone())
                {
                    ...
                    averageNormal += edgeIt.current()->to()->nor;
                    edgeIt.advance();
                }
                ...
                if (normalCache.size() > index)
                    normalCache[index] = SprueLerp(normalBuffer_[index], averageNormal, strength).Normalized();
                vertIt.advance();
            }
            positionBuffer_ = cache;
            normalBuffer_ = normalCache;
    ...
    std::shared_ptr<MeshData> MeshData::Decimate(float intensity) const
    {
        ...
        for (unsigned i = 0; i < indexBuffer_.size(); i += 3)
        {
            ...
            Simplify::triangles[i/3].v[0] = indices[0];
            Simplify::triangles[i/3].v[1] = indices[1];
            Simplify::triangles[i/3].v[2] = indices[2];

            Vec3 e0 = positionBuffer_[indices[1]] - positionBuffer_[indices[0]];
            Vec3 e2 = positionBuffer_[indices[2]] - positionBuffer_[indices[0]];
            Simplify::triangles[i/3].n = e0.Cross(e2).Normalized();
        }
        ...
        for (unsigned i = 0; i < Simplify::triangles.size(); ++i)
        {
            ...
            ret->indexBuffer_[i * 3 + 1] = Simplify::triangles[i].v[1];
            ret->indexBuffer_[i * 3 + 2] = Simplify::triangles[i].v[2];
        }
