...
void SprueEngine::MeshData::ComputeUVCoordinates(unsigned width, unsigned height, int quality, int charts, float stretch, float gutter, UV_CALLBACK callback)
{
    ...
    if (!FAILED(DirectX::GenerateAdjacencyAndPointReps(indexBuffer_.data(), indexBuffer_.size() / 3, (DirectX::XMFLOAT3*)positionBuffer_.data(), positionBuffer_.size(), EPSILON, 0, adjacency.data())))
    {
        ...
        {
            if (callback && callback(0) == E_ABORT)
                return;
