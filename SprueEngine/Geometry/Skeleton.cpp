            ...
            for (auto child : currentJoint->GetChildren())
            {
                if (child)
                {
                    if (currentJoint->AllowAutoweight())
                        boneList.push_back({ currentJoint->GetIndex(), child->GetIndex(), LineSegment(currentJoint->GetPosition(), child->GetPosition()) });//LineSegment(currentJoint->GetModelSpacePosition(), child->GetModelSpacePosition()) });
